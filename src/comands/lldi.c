...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/08 15:56:38 by rywisozk          #+#    #+#             */
/*   Updated: 2019/07/08 15:56:40 by rywisozk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		apply_lldi(t_carriage *carr, t_game *game, t_oper *oper)
{
	t_oper	*tmp;
	int		arg;
	int		arg2;
	...
	if (oper->arg1_type == T_IND)
		arg = get_arg_value(game, carr->curr_position + arg % IDX_MOD, 4);
...
void		exec_lldi(t_carriage *carr, t_game *game)
{
	...
	apply_lldi(carr, game, &oper);
	move_carr(carr, oper.move_len);
	if (game->verbose)
		print_lldi(game, oper);
}
