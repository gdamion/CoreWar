...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/08 15:56:13 by rywisozk          #+#    #+#             */
/*   Updated: 2019/07/08 15:56:15 by rywisozk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_arg1(t_game *game, t_oper *oper)
{
	...
	{
		get_t_dir_short_value(game, oper, 1);
...
void		apply_ldi(t_carriage *carr, t_game *game, t_oper *oper)
{
	t_oper *tmp;

	tmp = oper;
	if (oper->arg1_type == T_IND)
		oper->arg1 = get_arg_value(game, carr->curr_position +
		oper->arg1 % IDX_MOD, 4);
	else if (oper->arg1_type == T_REG)
		oper->arg1 = carr->regs[oper->arg1 - 1];
	if (oper->arg2_type == T_REG)
		oper->arg2 = carr->regs[oper->arg2 - 1];
	carr->regs[oper->arg3 - 1] = get_arg_value(game, carr->curr_position +
	((oper->arg1 + oper->arg2) % IDX_MOD), 4);
	oper->address_print = carr->curr_position + ((oper->arg1
	+ oper->arg2) % IDX_MOD);
}
